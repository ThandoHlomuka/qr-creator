...
async function deleteQrFromProfile(profileId, slug) {
  try {
    await redis.del(qrKey(slug));
    await redis.srem(LIST_KEY, slug);
    await redis.del(`qr:scans:${slug}`);
    await redis.srem(`profile:${profileId}:qrs`, slug);
    const count = await redis.scard(`profile:${profileId}:qrs`);
    await updateProfile(profileId, { qrCount: count });
  } catch {
    const err = new Error('Failed to delete QR code.');
    err.status = 500;
    throw err;
  }
}

module.exports = { createQr, getQr, updateQr, deleteQr, deleteQrFromProfile, recordScan, recordScanDetailed, recordShare, listQrs, createProfile, getProfile, updateProfile, saveQrToProfile, removeQrFromProfile, getProfileQrs, getScanLog, getQrAnalytics };
